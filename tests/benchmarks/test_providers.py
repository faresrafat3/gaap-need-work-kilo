...
async def test_all_providers(test_all_keys: bool = False):
    ...
    working_providers = [
        name for name, results in by_provider.items()
    ...
    if working_providers:
        ...
        for name in working_providers:
            ...
            avg_time = sum(r.response_time_ms for r in provider_results if r.success) / max(sum(1 for r in provider_results if r.success), 1)
