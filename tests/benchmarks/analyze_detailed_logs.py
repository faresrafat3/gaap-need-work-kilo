...
def analyze_accuracy_patterns(questions: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    different_answers = sum(
        ...
        if q["direct_answer"] != q["gaap_answer"]
    )
...
def analyze_performance_by_attempts(questions: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        "by_direct_attempts": {
            attempts: calc_stats(qs)
            for attempts, qs in sorted(by_direct_attempts.items())
        },
        "by_gaap_attempts": {
            attempts: calc_stats(qs)
            for attempts, qs in sorted(by_gaap_attempts.items())
        },
    }
...
def analyze_timeline(events: List[Dict[str, Any]], metadata: Dict[str, Any]) -> Dict[str, Any]:
    ...
    question_complete_events = [
        e for e in events 
        if e["type"] in ["direct_complete", "gaap_complete"]
    ]
    ...
    return {
        ...
        "avg_time_between_events": sum(question_times) / len(question_times) if question_times else 0,
...
def generate_analysis_report(run_dir: Path) -> str:
    ...
    lines = []
    lines.append("=" * 80)
    lines.append(f"DETAILED BENCHMARK ANALYSIS")
    ...
    lines.append(f"Duration: {metadata['total_duration_seconds']:.2f}s ({metadata['total_duration_seconds']/60:.2f} min)")
    ...
    if rl_analysis['total_retries'] > 0:
        total_retry_time = sum(
            info['total_delay'] 
            for info in rl_analysis['retries_per_question'].values()
        )
        lines.append(f"Total Time Waiting for Rate Limits: {total_retry_time:.2f}s ({total_retry_time/60:.2f} min)")
        lines.append(f"  {total_retry_time / metadata['total_duration_seconds'] * 100:.1f}% of total time")
    lines.append("")
    ...
    if rl_analysis['retries_per_question']:
        ...
        sorted_retries = sorted(
            rl_analysis['retries_per_question'].items(),
            key=lambda x: x[1]['count'],
            reverse=True
        )[:5]
        for context, info in sorted_retries:
            lines.append(f"  {context}: {info['count']} retries, {info['total_delay']:.1f}s total delay")
        lines.append("")
    ...
    lines.append(f"  - metadata.json: Run configuration")
    ...
    lines.append(f"  - SUMMARY.md: High-level summary")
    lines.append(f"  - DETAILED.md: Question-by-question report")
    lines.append(f"  - PERFORMANCE.md: Performance metrics")
    lines.append("")
