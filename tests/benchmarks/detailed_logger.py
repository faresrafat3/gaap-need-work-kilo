...
class DetailedBenchmarkLogger:
    ...
    def finalize(self):
        ...
        self.logger.info(f"Completed questions: {self.metadata.completed_questions}/{self.metadata.total_questions}")
    ...
    def _auto_archive_old_runs(self):
        ...
        try:
            ...
            enabled = os.getenv("GAAP_LOG_RETENTION_ENABLED", "1").strip().lower() not in {"0", "false", "no"}
            if not enabled:
                return
    ...
    def _generate_summary_report(self) -> str:
        ...
        lines.append(f"- Completed: {self.metadata.completed_questions}/{self.metadata.total_questions}")
        lines.append(f"- Direct Accuracy: {correct_direct}/{total} ({correct_direct/max(total,1)*100:.1f}%)")
        lines.append(f"- GAAP Accuracy: {correct_gaap}/{total} ({correct_gaap/max(total,1)*100:.1f}%)")
    ...
    def _generate_detailed_report(self) -> str:
        ...
        lines = []
        lines.append(f"# Detailed Question Report")
    ...
    def _generate_performance_report(self) -> str:
        ...
        lines.append(f"- Direct Min/Max: {min(direct_latencies):.0f}ms / {max(direct_latencies):.0f}ms")
    ...
    def _generate_readme(self) -> str:
        ...
        lines.append(f"- Dataset: {self.metadata.dataset}")
        lines.append(f"- Questions: {self.metadata.completed_questions}/{self.metadata.total_questions}")
