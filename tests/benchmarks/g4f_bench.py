...
def run_benchmark(
    ...
    for q_num, idx in enumerate(indices, start=1):
        ...
        try:
            ...
            print(
                f"  Q{q_num:3d}/{samples} {mark} [{base_model[:20]:<20}] "
    ...
    if model_correct:
        ...
        for model, count in sorted(
            model_total.items(), key=lambda x: -model_correct.get(x[0], 0) / max(x[1], 1)
        ):
    ...
    return {
        ...
        "cost_usd": 0.0,
        "per_model": {
            m: {"correct": model_correct.get(m, 0), "total": model_total.get(m, 0)}
...
def _run_battle_mode(prompt: str):
    ...
    print("  Note: Model will be randomly assigned (may be private/unreleased)")
    print("=" * 60)
    ...
    try:
        ...
        async def battle_request():
            ...
            try:
                ...
                updated_args, grecaptcha = await LMArena.get_args_from_nodriver(
                    proxy=None, force=True
                )
        ...
        if result:
            ...
            for name in [model_a_name, model_b_name]:
                if any(
                    p in name.lower() for p in ["gemini-3", "gemini-2.5-pro-preview", "unreleased"]
                ):
...
def main():
    ...
    parser.add_argument(
        "--delay", type=float, default=4.0, help="Min seconds between requests (default: 4.0)"
    )
    parser.add_argument(
        "--profile",
        type=str,
    ...
    parser.add_argument(
        "--list-chain", action="store_true", help="Show the fallback chain configuration and exit"
    )
    parser.add_argument(
        "--show-onboarding",
        action="store_true",
    ...
    parser.add_argument(
        "--preflight", action="store_true", help="Run provider health checks before benchmark"
    )
    parser.add_argument(
        "--auth", action="store_true", help="Warm up LMArena auth (opens browser for captcha)"
    )
    parser.add_argument(
        "--auth-status", action="store_true", help="Check LMArena auth cache status"
    )
    parser.add_argument(
        "--auth-reset", action="store_true", help="Delete LMArena auth cache and re-authenticate"
    )
    parser.add_argument(
        "--battle",
        type=str,
    ...
    if args.profile in ("premium", "quality"):
        ...
        if status["valid"]:
            ...
            print("      Run --auth to authenticate first")
