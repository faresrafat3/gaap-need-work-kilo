...
from compare_official import compare_results, generate_report
    ...
    analysis = {
        ...
        "accuracy": {
            ...
            "improvement_relative": (
                ((gaap.get("accuracy", 0) / direct.get("accuracy", 1)) - 1) * 100
                if direct.get("accuracy", 0) > 0
                else 0
            ),
        },
        "latency": {
            ...
            "overhead": gaap.get("latency_ms", {}).get("avg", 0)
            - direct.get("latency_ms", {}).get("avg", 0),
            "overhead_ratio": (
                gaap.get("latency_ms", {}).get("avg", 1)
                / direct.get("latency_ms", {}).get("avg", 1)
                if direct.get("latency_ms", {}).get("avg", 0) > 0
                else 0
            ),
        },
        "cost": {
...
def main():
    ...
    try:
        ...
        print("Error: Could not parse input as JSON or Python dict")
        return
