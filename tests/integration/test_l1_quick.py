...
async def main():
    ...
    # 3. Create L0 + L1
    layer0 = Layer0Interface(firewall_strictness="high", enable_behavioral_analysis=True)

    layer1 = Layer1Strategic(tot_depth=5, tot_branching=4, mad_rounds=3, provider=provider)
    ...
    print(f"\n{'=' * 60}")
    print(
        f"RESULT: {'✓ LLM STRATEGY' if spec.metadata.get('strategy_source') == 'llm' else '✗ FALLBACK'}"
    )
    print(f"{'=' * 60}")
