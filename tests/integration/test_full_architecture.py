...
# الاستيرادات
from gaap.gaap_engine import GAAPEngine, GAAPRequest, GAAPResponse
from gaap.providers.webchat_bridge import WebChatBridgeProvider, create_kimi_provider
...
from gaap.layers.layer1_strategic import Layer1Strategic
from gaap.layers.layer2_tactical import Layer2Tactical
    ...
    try:
        ...
        if response.task_graph:
            ...
            for task_id, node in list(graph.all_nodes.items())[:5]:
                ...
                print(f"    {C.DIM}• [{cat}] {task.name[:55]} ({src_badge}{C.RESET}{C.DIM}, deps={deps}){C.RESET}")
        ...
        if response.execution_results:
            ...
            avg_quality = sum(r.quality_score for r in response.execution_results) / max(exec_count, 1)
            ...
            if all_evals:
                ...
                for ev in all_evals[:4]:
                    ...
                    print(f"    {C.DIM}• {ev.critic_type.name}: {ev.score:.0f}/100 [{status}{C.RESET}{C.DIM}]{C.RESET}")
...
async def run_all_tests():
    ...
    print(f"  {C.BOLD}Layers:{C.RESET} L0 (Security+Classify) → L1 (ToT+MAD) → L2 (Decompose+DAG) → L3 (Execute+Twin+Quality)")
    print(f"  {C.BOLD}Systems:{C.RESET} Self-Healing, Hierarchical Memory, Prompt Firewall, Audit Trail")
    ...
    try:
        from gaap.providers.account_manager import PoolManager, auto_discover_accounts, bootstrap_pools
        ...
        if kimi_pool.accounts:
            ok(f"Kimi AccountPool: {len(kimi_pool.accounts)} accounts "
    ...
    try:
        ...
        t_smoke = time.time()
        result = webchat_call("kimi", [{"role": "user", "content": "Say 'GAAP ready' in exactly 2 words."}], "kimi-k2.5-thinking", timeout=60)
    ...
    ok(f"Layer 0: IntentClassifier + PromptFirewall")
    ok(f"Layer 1: ToT(depth={engine.layer1.tot.max_depth}, branch={engine.layer1.tot.branching_factor}) + MAD({engine.layer1.mad_panel.max_rounds}r)")
    has_llm = engine.layer2.decomposer._provider is not None
    ok(f"Layer 2: TacticalDecomposer(max={engine.layer2.decomposer.max_subtasks}, LLM={'ON' if has_llm else 'OFF'})")
    ok(f"Layer 3: ExecutorPool + GeneticTwin(on={engine.layer3.twin_system.enabled}) + MADQualityPanel(min={engine.layer3.mad_panel.min_score})")
    ...
    for r in all_results:
        ...
        print(f"       {C.DIM}Layers: {layers} | Time: {t:.0f}ms | Quality: {quality:.0f}/100{C.RESET}")
        if r.get("error"):
    ...
    expected_layers = {"L0_INTERFACE", "L0_SECURITY_BLOCK", "L1_STRATEGIC", "L2_TACTICAL", "L3_EXECUTION"}
    ...
    metric("Architecture Coverage", f"{coverage:.0f}% ({len(covered)}/{len(expected_layers)} layers)")
    ...
    report_path = os.path.join(
        ...
        "benchmark_logs",
        f"full_arch_test_{int(time.time())}.json"
    )
