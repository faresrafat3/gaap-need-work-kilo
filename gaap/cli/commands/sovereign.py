...
async def cmd_research(args: Any):
    """Deep Research Command"""
    print(f"🔍 Starting Deep Research on: {args.query} (Depth: {args.depth})")
    engine = create_engine()
    request = GAAPRequest(
        text=args.query, 
        metadata={"force_intent": "RESEARCH", "research_depth": args.depth}
    )
