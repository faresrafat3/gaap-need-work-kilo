...
class MCTSConfig:
    ...
    Attributes:
        ...
        exploration_weight: UCT exploration parameter (default: 1.414)
    ...
    def for_priority(cls, priority: TaskPriority) -> "MCTSConfig":
        ...
        CRITICAL: 50+ iterations (no catastrophic failure branches missed)
        HIGH: 30 iterations
...
class MCTSNode:
    ...
    def uct_score(self, exploration_weight: float = 1.414) -> float:
        ...
        UCT = Q(s,a) + C * sqrt(ln(N(parent)) / N(s))
    ...
    def get_path(self) -> Sequence["MCTSNode"]:
        ...
        Returns:
            Sequence of nodes from root to this node
        """
    ...
    def get_most_visited_child(self) -> "MCTSNode | None":
        """
        Get the most visited child (best after search).
...
class ValueOracle:
    """
    Value Oracle: Predicts success probability of a node.

    The Oracle estimates the "Success Probability" of a decision node.
    Uses both heuristic scoring and LLM-based evaluation when available.

    Attributes:
        provider: LLM provider for value estimation
        model: Model to use for LLM evaluation
        _heuristic_weights: Weights for heuristic scoring
    """
...
No explanation, no other text."""
    ...
    async def _llm_evaluate(self, node: MCTSNode, context: dict[str, Any], intent: Any) -> float:
        ...
        Returns:
            LLM-predicted success probability
        """
    ...
    def _parse_value(self, raw: str) -> float:
        """
        Parse LLM response to float value.

        Args:
            raw: Raw LLM response

        Returns:
            Parsed float value (0.0 to 1.0)
        """
        import re
...
class MCTSStrategic:
    ...
    async def search(
        ...
        Returns:
            Tuple of (best path root node, statistics)
        """
        context = context or {}
    ...
    async def _generate_actions(
        ...
        """
        level = node.depth
        actions = self._get_default_actions(level)
        return actions
    ...
    async def _simulator_rollout(
        ...
        """
        Rollout using the simulator.

        Args:
            node: Node to rollout from
            intent: Intent for simulation
            context: Context
    ...
    async def _oracle_rollout(self, node: MCTSNode, intent: Any, context: dict[str, Any]) -> float:
        """
        Rollout using the oracle.

        Args:
            node: Node to rollout from
            intent: Intent for evaluation
            context: Context

        Returns:
            Oracle-evaluated reward
        """
...
def create_mcts_for_priority(
    ...
    Returns:
        MCTSStrategic configured for the priority
    """
